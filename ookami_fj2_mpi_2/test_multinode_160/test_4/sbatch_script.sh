...
for nodes in {32..1..-1} #25 26 27 28 30 32 #{1..24} #{32..1..-1} # {1..8}
do
partition_param="--partition short "
if [[ $nodes -gt 1 ]]
then
...
do
	...
	sbatch $partition_param -n $tasks -N $nodes --exclusive --ntasks-per-node=$numa --time=0-00:30:00  run_mpi.sh $size $threads 
	done
